...
-- Returns total UNREAD MESSAGE counts for the calling user (sum of the
-- per-thread unread numbers, so the sidebar overall badge = the same total).
...
--   user_unread  : total staff messages a resident hasn't seen
--   admin_unread : total resident messages staff haven't seen across
--                  non-archived threads (only returned to staff; 0 for residents)
