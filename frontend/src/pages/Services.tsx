  ...
  return (
    <div className="bg-background font-body-md text-on-background antialiased min-h-screen overflow-x-hidden">
      ...
      <main className="pt-20 w-full flex flex-col">
        <section className="relative py-12 md:py-xl px-4 md:px-margin-desktop flex flex-col items-center text-center w-full">
          ...
          <div className="relative z-10 max-w-4xl mx-auto w-full flex flex-col items-center">
            <span className="inline-block px-md py-xs bg-secondary/10 text-secondary rounded-full font-label-md text-xs md:text-label-md mb-md">Resident Support System</span>
            <h1 className="font-display-lg text-3xl md:text-display-lg text-primary mb-md">Law Enforcement &amp; Public Safety System</h1>
            <p className="font-body-lg text-base md:text-body-lg text-on-surface-variant mb-lg max-w-2xl mx-auto">An integrated platform for real-time incident reporting, tactical coordination, and community enforcement services.</p>
            <div className="flex flex-col sm:flex-row gap-md justify-center items-center w-full max-w-md md:max-w-none">
              <button
                 className="px-lg py-md bg-secondary text-on-secondary rounded-xl font-label-md text-label-md shadow-lg flex items-center justify-center gap-sm whitespace-nowrap transition-transform hover:scale-105 active:scale-95"
                onClick={() => {
                  if (!user) {
              ...
              <button className="px-lg py-md border-2 border-secondary text-secondary rounded-xl font-label-md text-label-md bg-white/50 backdrop-blur-sm whitespace-nowrap transition-all hover:bg-secondary/5">View System Guide</button>
            </div>
          </div>
        </section>

        <section className="py-12 md:py-xl px-4 md:px-margin-desktop bg-surface w-full flex flex-col">
          <div className="flex flex-col md:flex-row justify-between items-center md:items-end mb-lg gap-md w-full">
            <div className="max-w-ml text-center md:text-left">
              <h2 className="font-headline-lg text-2xl md:text-headline-lg text-primary mb-xs">Frontline Assistance</h2>
              ...
              <div key={i} className="glass-card p-md rounded-2xl flex flex-col justify-between w-full border border-outline-variant/30 bg-surface-bright/50 backdrop-blur-sm shadow-sm hover:shadow-lg transition-all duration-700">
                <div>
                  <div className="w-14 h-14 bg-secondary/10 rounded-xl flex items-center justify-center mb-md">
                    <span className="material-symbols-outlined service-icon text-[32px] text-secondary" style={{ fontVariationSettings: '"FILL" 1' }}>{svc.icon}</span>
                  </div>
        ...
        <section ref={(el) => { sectionRefs.current[1] = el; }} className="px-4 md:px-margin-desktop py-12 md:py-xl w-full opacity-0 translate-y-10 transition-all duration-700">
          <div className="qc-services-banner rounded-[2.5rem] overflow-hidden shadow-2xl flex flex-col lg:flex-row items-stretch w-full bg-on-background relative">
            ...
            <div className="flex-1 p-8 md:p-lg flex flex-col justify-center items-center text-center w-full relative z-10">
              ...
              <h2 className="font-headline-lg text-2xl md:text-headline-lg text-surface-bright mb-md">Tactical Command Integration</h2>
              <p className="font-body-lg text-base md:text-body-lg text-on-primary-container leading-relaxed mb-lg">Our system bridges the gap between citizens and the Barangay Public Safety Officers (BPSO), providing a unified dashboard for localized enforcement and incident management.</p>
              <div className="flex flex-col sm:flex-row gap-md justify-center w-full md:w-auto">
                <Link className="inline-flex items-center justify-center gap-sm bg-white text-primary px-lg py-md rounded-xl font-bold hover:scale-105 transition-transform" to="#">Access Officer Portal <span className="material-symbols-outlined">admin_panel_settings</span></Link>
                <Link className="inline-flex items-center justify-center gap-sm text-surface-bright border border-white/20 px-lg py-md rounded-xl font-label-md text-label-md hover:bg-white/5 transition-colors" to="#">System Documentation</Link>
              </div>
            </div>
            <div className="flex-1 min-h-[250px] md:min-h-[300px] relative w-full overflow-hidden">
        ...
        <section ref={(el) => { sectionRefs.current[2] = el; }} className="py-12 md:py-xl px-4 md:px-margin-desktop bg-surface-container-low/30 w-full flex flex-col opacity-0 translate-y-10 transition-all duration-700">
          ...
          <div className="max-w-4xl mx-auto grid grid-cols-1 md:grid-cols-2 gap-md w-full">
            <div className="p-lg bg-surface-bright rounded-2xl border border-outline-variant/30 hover:shadow-lg transition-shadow">
