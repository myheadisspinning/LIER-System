...
import { assessIncident, checkDuplicateReport, classifyIncident, insertIncidentReport, uploadEvidence, type AiAnalysis, type EvidenceFile } from '../../../lib/ai';
...
// Broad incident vocabulary used to decide whether the title/description
...
// so the AI tactical analysis is not triggered on nonsense.
function looksLikeIncident(title: string, description: string) {
...
export default function ReportIncident({ className = '' }: { className?: string }) {
 ...
 const handleSubmit = async () => {
  ...
  try {
   ...
   // Automatically run the AI Credibility check while evidence
   // uploads, so the admin desk sees the verdict the moment the report
   // lands. Non-fatal: if the AI check fails, the report still submits and
   // an admin can re-run the check from the admin desk later.
   const assessmentPromise = assessIncident({
    title: reportTitle.trim(),
    description: [reportDescription.trim(), additionalContext.trim()].filter(Boolean).join(' '),
   ...
   const ai_assessment = await assessmentPromise;
 ...
 const goToStep = (target: number) => {
  if (step === 1) {
   if (!category) {
    ...
    scrollToField(categoryDropdownRef.current);
    return;
   }
   if (category === 'Other/Uncategorized' && !customCategory.trim()) {
    setMissingField('category');
 ...
 return (
  <div className={`bg-surface w-full h-full ${className} overflow-hidden flex flex-col relative`}>
   {/* Critical Notice */}
     ...
     <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 sm:gap-6">
        ...
        <div>
         ...
         <div className="relative" ref={categoryDropdownRef}>
          <button
           ...
           className={`w-full bg-white border rounded-lg py-[10px] px-3 pr-10 font-label-md text-label-md text-left focus:outline-none focus:border-secondary transition-all cursor-pointer ${
            category ? 'text-on-surface' : 'text-on-surface-variant italic'
         ...
         <div className="flex bg-white rounded-lg p-1 border border-outline-variant">
          {['Ongoing', 'Happened', 'Unconfirmed'].map((s) => (
           <button
            ...
            className={`flex-1 py-2 text-center rounded-md font-label-sm text-label-sm transition-colors ${incidentStatus === s ? 'bg-surface border border-outline-variant text-secondary font-bold' : 'text-on-surface-variant hover:bg-surface-container-low'}`}
           >
            {s}
           </button>
          ))}
      ...
      <div className="flex flex-col rounded-xl border border-outline-variant/30 relative overflow-hidden">
       ...
       <div className="relative w-full min-h-[280px] sm:min-h-[460px] bg-surface-container-low">
        ...
        {/* legend */}
        <div className="absolute bottom-3 left-3 sm:bottom-auto sm:left-auto sm:top-4 sm:right-4 z-[600] rounded-lg bg-surface/95 border border-outline-variant px-2 py-1 sm:px-3 sm:py-2 text-[8px] sm:text-[9px] font-bold text-on-surface items-center gap-x-1.5 sm:flex-col sm:items-start sm:gap-y-1.5">
         <div className="flex items-center gap-1.5"><span className="w-2 h-2 rounded-full bg-error"></span> Incident</div>
         <div className="flex items-center gap-1.5 sm:mb-1.5"><span className="w-2 h-2 rounded-full bg-secondary"></span> Unit</div>
         <div className="flex items-center gap-1.5"><span className="w-2 h-0.5 border-t-2 border-dashed border-on-surface-variant/50"></span> District</div>
        </div>
    ...
    {step === 3 && (
     <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 sm:gap-6">
      <div className="lg:col-span-2 space-y-4 sm:space-y-6">
       <div className="bg-surface border border-outline-variant/30 rounded-lg p-4 sm:p-6 relative overflow-hidden">
      ...
      <div className="space-y-4 sm:space-y-6">
       <div className="bg-surface border border-outline-variant/30 rounded-2xl p-4 sm:p-6 lg:sticky lg:top-24">
