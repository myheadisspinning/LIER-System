...
export default function SignIn() {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (remember) {
      localStorage.setItem('remember_email', identity);
    } else {
            ...
            <div className="max-w-2xl w-full mx-auto my-auto py-4">
              ...
              <form className="space-y-4" onSubmit={handleSubmit}>
                ...
                <div className="flex items-center gap-2 py-1">
                  <input className="w-5 h-5 rounded border-outline-variant text-secondary focus:ring-secondary/20"
                    id="remember" type="checkbox" checked={remember}
                    onChange={(e) => setRemember(e.target.checked)} />
                  <label className="text-caption text-on-surface-variant cursor-pointer select-none" htmlFor="remember">
              ...
              {showResendOption && (
                <div className="mt-4 p-4 bg-warning-amber/10 border border-warning-amber/30 rounded-lg">
                  <p className="text-body-sm text-on-surface mb-3">
                    Didn't receive the confirmation email? Click below to resend it.
                  </p>
                  <button
                    type="button"
                    onClick={handleResendConfirmation}
                    disabled={resending}
                    className="w-full bg-warning-amber text-on-warning rounded-lg font-semibold text-body-sm shadow-sm hover:bg-warning-amber/90 transition-all active:scale-[0.98] flex items-center justify-center gap-2 py-2 disabled:opacity-50"
                  >
                    {resending ? (
                      <>
              ...
              <div className="text-center">
                <p className="text-body-md text-on-surface-variant">
                  ...
                  <Link className="text-secondary font-semibold hover:underline" to="/signup">Register here</Link>
                </p>
              </div>
            </div>
            {/* Footer */}
